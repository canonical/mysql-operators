...
class MySQLBase(ABC):
    ...
    def execute_backup_commands(
        ...
        try:
            ...
            tmp_dir_backup, _ = self._execute_commands(make_temp_dir_command, user=user, group=group)
            ...
            if ca_chain:
                ...
                write_ca_file_content_command = f"echo {ca_file_content_b64} | base64 -d > {ca_file_location}".split()
    ...
    def retrieve_backup_with_xbcloud(
        ...
        make_ca_dir_command = (
            f"mktemp --directory {temp_restore_directory}/#s3_ca_XXXX".split()
        )
        ca_chain = s3_parameters.get("tls-ca-chain")
        try:
            nproc, _ = self._execute_commands(nproc_command)
