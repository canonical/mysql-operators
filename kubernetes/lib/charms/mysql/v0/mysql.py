...
class MySQLCharmBase(CharmBase, ABC):
    ...
    def _on_promote_to_primary(self, event: ActionEvent) -> None:
        ...
        if scope not in {"unit", "cluster"}:
            message = "The action requires the scope parameter to be set to either 'unit' or 'cluster'"
            event.fail(message)
            logger.info(message)
            return
    ...
    def unit_initialized(self, raise_exceptions: bool = False) -> bool:
        ...
        if not self.app_peer_data.get("cluster-name"):
            # early calls - cluster name not set, avoid KeyError
            return False
        try:
...
class MySQLBase(ABC):
    ...
    def _release_all_locks(self, executor: BaseExecutor):
        ...
        try:
            ...
            logger.warning(
                "Failed to release all locks. Manual intervention may be required to release locks."
            )
