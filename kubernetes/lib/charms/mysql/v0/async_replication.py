...
class MySQLAsyncReplication(Object):
    ...
    def on_async_relation_broken(self, event: RelationBrokenEvent):  # noqa: C901
        ...
        if (
            self.role.cluster_role in (ClusterRole.REPLICA, "UNSET")
        ...
        elif self.role.cluster_role == ClusterRole.PRIMARY:
...
class MySQLAsyncReplicationConsumer(MySQLAsyncReplication):
    ...
    def _on_consumer_changed(self, event):  # noqa: C901
        ...
        if state == States.SYNCING:
            ...
            self._charm.unit_peer_data["member-state"] = InstanceState.ONLINE
            self._charm.unit_peer_data["member-role"] = InstanceRole.PRIMARY
            event.defer()
