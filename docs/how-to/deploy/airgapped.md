...
The [`snap.yaml`](https://github.com/canonical/mysql-bundle/blob/main/releases/latest/) shipped by the MySQL bundle is mapped to the published [`bundle.yaml`](https://github.com/canonical/mysql-bundle/blob/main/releases/latest/).
```
...
:sync: vm
    ...
    store-admin export bundle mysql-bundle --channel=8.4/edge --base=ubuntu@24.04 --arch=amd64
...
```{tip}
Always check the [checksum](https://en.wikipedia.org/wiki/Checksum) for the transferred blobs!
```
