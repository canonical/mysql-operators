...
class ContainerRuntime(BaseRuntime):
    ...
    def _parse_memory(self, memory: str) -> int:
        ...
        memory_value = parts.groups()[0]
        memory_units = parts.groups()[1].upper()
        return int(memory_value) * self.memory_bytes[memory_units]
    ...
    def get_cores(self) -> int:
        ...
        try:
            ...
            raise RuntimeError("Failed to fetch core information") from e
    ...
    def get_memory(self) -> int:
        ...
        try:
            ...
            raise RuntimeError("Failed to fetch memory information") from e
    ...
    def get_hostname(self, address: str) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to resolve FQDN for {address}") from e
    ...
    def check_service(self, address: str, port: int) -> None:
    ...
    def create_service(self, name: str, labels: Mapping[str, str]) -> None:
        ...
        In case of failure, it must not leak the lightkube exception.
        That way we can reduce the coupling with the lightkube package.
        """
        logger.debug(f"Creating K8s service {name}")
        ...
        try:
            self._client.create(service)
        except ApiError as e:
            if e.status.code == 409:
                logger.warning(f"Failed to create service: service already exists")
                return
            raise RuntimeError(f"Failed to create service") from e

    def update_labels(self, name: str, labels: Mapping[str, str]) -> None:
