...
class K8sServerService(BaseServerService):
    ...
    def running(self) -> bool:
        ...
        return all((
            ...
            self._container.get_service(self.name).is_running(),
        ))
    ...
    def initialized(self) -> bool:
        ...
        if not self._container.can_connect():
            logger.warning("Skipping initialization check: container is not ready")
            return False

        data_path = str(self._system.paths.mysql_data)
        data_files = list(self._container.list_files(data_path))
    ...
    def _generate_layer_env(self) -> dict[str, str]:
        ...
        The returned values must be stable across invocations, given that
        this environment is used within the Pebble layers, and any difference
        makes pebble to run a reconciliation loop that restarts the MySQL Server
        """
        ...
        if external_http_proxy or external_https_proxy:
            ...
            internal_proxy_entries.append(internal_domain)
            internal_proxy_entries.sort()
