...
class BackupManager:
    ...
    def check_cluster(self) -> None:
        ...
        status = self._clients.cluster.fetch_status()
    ...
    def prepare_instance(self, setup: bool) -> None:
        ...
        self._clients.cluster.set_instance_option("tag:_hidden", setup_flag)

        try:
            logger.info("Setting instance offline")
            self._clients.instance.update_variable("offline_mode", offline_flag)
        except ExecutionError:
            self._clients.cluster.set_instance_option("tag:_hidden", clean_flag)
            raise
