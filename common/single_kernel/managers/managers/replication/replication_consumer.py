...
class ConsumingReplicationManager(BaseReplicationManager):
    ...
    def get_status(self, target_members: int) -> ReplicationStatus:
        ...
        local_address = self._state_local.get_instance_address()
    ...
    def check_cluster_version(self) -> None:
        ...
        try:
            remote_version = self._state_remote.get_cluster_version()
    ...
    def update_cluster_credentials(self, usernames: dict[str, str]) -> None:
        ...
        for key, password in self._state_remote.secrets.get_secret_content(secret_id).items():
            self._clients.instance.update_user(usernames[key], password)
