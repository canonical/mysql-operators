...
class ConsumingReplicationManager(BaseReplicationManager):
    ...
    def get_status(self, target_members: int) -> ReplicationStatus:
        ...
        remote_secret = self._state_remote.app.get_secret_id()
        local_address = self._state_local.app.get_instance_address()

        # Credentials exists in the primary cluster but not yet synchronized
        if remote_secret and not local_address:
            return ReplicationStatus.SYNCING

        # Cluster not yet initialized
        if not self._state_local.app.get_replica_flag():
            return ReplicationStatus.INITIALIZING

        online_members = self._clients.instance.count_cluster_members(states=[InstanceState.ONLINE])
    ...
    def check_instance_data(self) -> None:
        ...
        try:
            databases = self._clients.instance.fetch_databases()
        except ExecutionError as e:
            raise RuntimeError("Failed to fetch instance databases") from e
    ...
    def check_cluster_version(self) -> None:
        ...
        try:
            remote_version = self._state_remote.app.get_cluster_version()
            local_version = self._clients.instance.fetch_version()
        except ExecutionError as e:
            raise RuntimeError("Failed to check cluster version") from e
    ...
    def dissolve_cluster(self) -> None:
        ...
        cluster_name = self._state_local.app.get_cluster_name()
        if not cluster_name:
            raise RuntimeError("Cluster name not found")

        try:
            self._clients.cluster_set.dissolve_cluster(cluster_name, force=True)
        except ExecutionError as e:
            raise RuntimeError("Failed to dissolve cluster") from e
    ...
    def wait_cluster_dissolving(self) -> None:
        ...
        try:
            for attempt in tenacity.Retrying(
                ...
                with attempt:
                    if all((
                        cluster_name in self._clients.instance.fetch_cluster_labels(),
                        cluster_name in self._clients.cluster_set.fetch_clusters(),
                    )):
