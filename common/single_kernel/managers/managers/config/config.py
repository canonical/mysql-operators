...
from .config_profile import Profile, ProfileConfigHelper
...
class ConfigManager:
    ...
    def __init__(self, state: None, system: BaseSystem, clients: Clients):
        ...
        self._profile_helper = ProfileConfigHelper(system.paths)
    ...
    def build_server_config(
        ...
        profile_config = self._profile_helper.get_config(memory_profile, memory, max_connections)
    ...
    def calculate_buffer_pool(self) -> int:
        """Calculate the MySQL server buffer pool."""
        memory = self._system.runtime.get_memory()
        params = self._profile_helper.calculate_buffer_pool(memory)

        return params[0]

    def read_server_config(self) -> dict:
        """Reads the MySQL server config."""
        string = self._system.paths.mysql_config_custom.read_text()
        parser = configparser.ConfigParser(interpolation=None)
        parser.read_string(string)

        return dict(parser["mysqld"])
    ...
    def check_static_key(self, keys: set[str]) -> bool:
        ...
        all_static_keys = (
            ...
            | self._profile_helper.static_keys
    ...
    def update_dynamic_var(self, key: str, val: Any) -> None:
        """Update a MySQL server dynamic variable."""
        try:
            self._clients.instance.update_variable(key, val)
        except ExecutionError as e:
            raise RuntimeError(f"Failed to update dynamic variable {key}") from e
