...
class ProfileConfigHelper:
    """Class to deal with the MySQL server profile config."""
    ...
    def _get_production_config(self, memory: int, max_connections: int | None) -> dict:
        """Return the operator configuration for the production profile."""
        if max_connections:
        ...
        (
            ...
            pool_chunk_size,
            message_cache_size,
        ) = self.calculate_buffer_pool(memory)
        ...
        if not max_connections:
            max_connections = max(self.calculate_max_connections(memory), MIN_CONNS)
    ...
    def _get_testing_config(self) -> dict:
        """Return the operator configuration for the testing profile."""
        return {
    ...
    @staticmethod
    def calculate_buffer_pool(memory: int) -> tuple[int, int | None, int | None]:
    ...
    @staticmethod
    def calculate_max_connections(memory: int) -> int:
    ...
    def get_config(self, profile: Profile, memory: int, max_connections: int | None) -> dict:
        """Return the operator profile specific config."""
        match profile:
            case Profile.PROD:
