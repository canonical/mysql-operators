...
class OperatorManager:
    ...
    def __init__(self, state: OperatorState, system: BaseSystem, clients: Clients):
    ...
    def cluster_initialized(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"The cluster is not yet initialized: {e}")
            return False
    ...
    def check_instance_join_cluster(self, instance_label: str) -> None:
        ...
        if instance_label in self._clients.cluster.fetch_instances():
            raise RuntimeError("The instance is already part of the cluster")
        if self.cluster_max_size == self._clients.instance.count_cluster_members():
            raise RuntimeError("The cluster cannot contain more members")
