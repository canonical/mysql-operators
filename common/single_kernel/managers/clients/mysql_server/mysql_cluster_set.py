...
class MySQLClusterSetClient:
    ...
    def __init__(self, executor: BaseExecutor, cluster_set_name: str):
        """Initialize the class attributes."""
        self._executor = executor
        self._client = ClusterClient(executor)
        self._cluster_set = cluster_set_name

    def create(self, cluster_name: str) -> None:
        """Create the MySQL cluster-set."""
        self._client.create_cluster_set(
            cluster_name=cluster_name,
            cluster_set_name=self._cluster_set,
        )
    ...
    def fetch_primary_host(self) -> str | None:
        ...
        if cluster_state == ClusterGlobalState.INVALIDATED:
            logger.warning("Cluster-set does not have quorum")
            return None
