...
class DatabaseEventHandler(Object):
    ...
    def __init__(self, charm: Operator, manager: DatabaseManager):
    ...
    def _database_requested(self, event: DatabaseRequestedEvent) -> None:
        ...
        try:
            ...
            self._manager.create_database_user(
                database=database,
                username=username,
    ...
    def _database_relation_broken(self, event: RelationBrokenEvent) -> None:
        """Event handler for the relation-broken event."""
        if not self._charm.unit.is_leader():
            return

        if self._charm.unit_departing:
            return
    ...
    def _database_relation_departed(self, event: RelationDepartedEvent) -> None:
        """Event handler for the relation-departed event."""
        if not self._charm.unit.is_leader():
            return
        if self._charm.app.name == event.departing_unit.app.name:
            return
