...
class TLSEventHandler(Object, ABC):
    ...
    def __init__(self, charm: Operator, manager: TLSManager, relation: str):
        ...
        self.framework.observe(
            ...
            self._certificate_available,
        )
        self.framework.observe(
            self._charm.on[relation].relation_broken,
            self._relation_broken,
        )
    ...
    def _certificate_available(self, event: CertificateAvailableEvent) -> None:
    ...
    def _relation_broken(self, _: RelationBrokenEvent) -> None:
        """Event handler for the relation-broken event."""
        if self._charm.unit_departing:
            logger.debug("Skipping TLS cleanup")
            return
