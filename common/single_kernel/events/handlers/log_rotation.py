...
from ops.charm import RelationBrokenEvent, RelationCreatedEvent, UpdateStatusEvent
...
class LogRotationEventHandler(Object):
    ...
    def __init__(
        ...
        self.framework.observe(self._charm.on.update_status, self._on_update_config)
    ...
    def _on_relation_created(self, event: RelationCreatedEvent) -> None:
        ...
        if not self._charm.initialized:
            logger.warning("Deferring logging configuration: charm not initialized")
            event.defer()
            return
    ...
    def _on_update_config(self, _: UpdateStatusEvent) -> None:
