...
class ReplicationData(BaseData):
    """Class to deal with the async replication."""
    ...
    def get_cluster_name(self) -> str | None:
        """Get the offering / consuming cluster name."""
    ...
    def get_cluster_version(self) -> str | None:
        """Get the offering cluster MySQL version."""
        version = self._data.get(self.cluster_version_key)
        if not version:
            return None
    ...
    def get_instance_address(self) -> str | None:
        """Get the offering cluster primary instance address."""
        endpoint = self._data.get(self.instance_address_key)
        if not endpoint:
            return None
    ...
    def get_instance_label(self) -> str | None:
        """Get the consuming cluster primary instance label."""
        label = self._data.get(self.instance_label_key)
        if not label:
            return None
    ...
    def get_replication_ready_flag(self) -> bool | None:
        """Get the offering cluster replication ready flag."""
        flag = self._data.get(self.replication_ready_key)
        if not flag:
            return None
    ...
    def get_replica_flag(self) -> bool | None:
        """Get the consuming cluster replica initialized flag."""
        flag = self._data.get(self.replica_key)
        if not flag:
            return None
    ...
    def get_secret_id(self) -> str | None:
        """Get the offering cluster secret ID holding the connection data."""
        secret_id = self._data.get(self.secret_key)
        if not secret_id:
            return None
    ...
    def get_switchover_count(self) -> int:
        """Get the offering / consuming cluster switchover count."""
        count = self._data.get(self.switchover_key)
        if not count:
            return 0
