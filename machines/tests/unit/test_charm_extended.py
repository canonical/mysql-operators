...
from mysql_vm_helpers import SnapServiceOperationError
...
class TestCharmExtended(unittest.TestCase):
    ...
    def test_get_unit_address_no_peers(self):
        with patch("charm.MySQLOperatorCharm.peers", new_callable=PropertyMock, return_value=None):
    ...
    @patch("charm.MySQLOperatorCharm._mysql", new_callable=PropertyMock)
    def test_handle_online_no_quorum_leader(self, _mysql):
        ...
        with (
            ...
            patch.object(self.charm.unit, "is_leader", return_value=True),
        ):
    ...
    @patch("charm.MySQLOperatorCharm._mysql", new_callable=PropertyMock)
    def test_handle_online_no_quorum_reboot_error(self, _mysql):
        _mysql.return_value.is_cluster_in_no_quorum.return_value = True
    ...
    @patch("charm.MySQLOperatorCharm._mysql", new_callable=PropertyMock)
    def test_handle_unreachable_restart_success(self, _mysql):
        with patch("charm.snap_service_operation", return_value=True):
            result = self.charm._handle_non_online_instance_status(InstanceState.UNREACHABLE)
        self.assertTrue(result)

    @patch("charm.MySQLOperatorCharm._mysql", new_callable=PropertyMock)
    def test_handle_unreachable_restart_fail(self, _mysql):
        with patch("charm.snap_service_operation", return_value=False):
            result = self.charm._handle_non_online_instance_status(InstanceState.UNREACHABLE)
        self.assertFalse(result)
        self.assertIsInstance(self.charm.unit.status, BlockedStatus)

    @patch("charm.MySQLOperatorCharm._mysql", new_callable=PropertyMock)
    def test_handle_unreachable_snap_error(self, _mysql):
        with patch("charm.snap_service_operation", side_effect=SnapServiceOperationError("fail")):
            result = self.charm._handle_non_online_instance_status(InstanceState.UNREACHABLE)
        self.assertFalse(result)
        self.assertIsInstance(self.charm.unit.status, BlockedStatus)
