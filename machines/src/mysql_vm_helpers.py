...
class MySQL(MySQLBase):
    ...
    def uninstall_mysql() -> None:
        """Uninstall the MySQL snap.

        Raises:
            MySQLUninstallError: if there is an error uninstalling the snap
        """
        try:
            cache = snap.SnapCache()
            charmed_mysql = cache[CHARMED_MYSQL_SNAP_NAME]
            ...
            if not charmed_mysql.present:
                logger.warning(f"{CHARMED_MYSQL_SNAP_NAME} snap not installed, skipping uninstall")
                return

            logger.info(f"Uninstalling {CHARMED_MYSQL_SNAP_NAME} snap")
            charmed_mysql.ensure(snap.SnapState.Absent)
        except snap.SnapError as e:
            logger.exception(f"Failed to uninstall {CHARMED_MYSQL_SNAP_NAME} snap")
            raise MySQLUninstallError from e

    @staticmethod
    def safe_uninstall_mysql() -> None:
        """Safely uninstall the MySQL snap.

        Unmounts the common directory before uninstalling the snap to avoid
        data loss or mount issues.

        Raises:
            MySQLUninstallError: if there is an error during unmount or uninstall
        """
        try:
            logger.info(f"Unmounting {CHARMED_MYSQL_COMMON_DIRECTORY}")
            # Parameters are generated by the charm
            ...
            logger.exception(f"Failed to unmount {CHARMED_MYSQL_COMMON_DIRECTORY}")
            raise MySQLUninstallError from e

        MySQL.uninstall_mysql()

    @staticmethod
    def reinstall_mysql() -> None:
        """Reinstall the MySQL snap.

        Safely uninstalls and then reinstalls the snap.

        Raises:
            MySQLUninstallError: if there is an error uninstalling the snap
            MySQLInstallError: if there is an error reinstalling the snap
        """
        MySQL.safe_uninstall_mysql()
        try:
            path = pathlib.Path(CHARMED_MYSQL_COMMON_DIRECTORY)
            if not path.exists():
                logger.debug(
                    f"{CHARMED_MYSQL_COMMON_DIRECTORY} does not exist, creating directory"
                )
                path.mkdir(parents=True, exist_ok=True)
            logger.info(f"Re-mounting {CHARMED_MYSQL_COMMON_DIRECTORY}")
            subprocess.check_call([  # noqa: S603
                "/usr/bin/mount",
                CHARMED_MYSQL_COMMON_DIRECTORY,
            ])
        except subprocess.CalledProcessError as e:
            logger.exception(f"Failed to mount {CHARMED_MYSQL_COMMON_DIRECTORY}")
            raise MySQLUninstallError from e
        logger.info(f"Reinstalling {CHARMED_MYSQL_SNAP_NAME} snap")
