...
class MySQL(MySQLBase):
    ...
    def _set_root_ownership(self, directory):
        ...
        """
        logger.debug(
            f"Setting ownership to {ROOT_SYSTEM_USER}:{ROOT_SYSTEM_USER} for MySQL operation"
        )
        try:
            subprocess.check_call([  # noqa: S603
                ...
                "chown",
                "-R",
                f"{ROOT_SYSTEM_USER}:{ROOT_SYSTEM_USER}",
                directory,
            ])
            subprocess.check_call([  # noqa: S603
                "/usr/bin/sudo",
            ...
            logger.error(f"Failed to set root ownership on data directory: {e}")
            raise

        try:
            yield
        finally:
            logger.debug(f"Restoring ownership to {MYSQL_SYSTEM_USER}:{ROOT_SYSTEM_USER}")
            try:
                subprocess.check_call([  # noqa: S603
                    "/usr/bin/sudo",
                    "chown",
                ...
                logger.error(f"Failed to restore snap_daemon ownership: {e}")
